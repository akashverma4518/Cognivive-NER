...
exports.getAlerts = exports.overrideDifficulty = exports.getPatientReminders = exports.getPatientGames = exports.getPatientTrends = exports.getPatientDetail = exports.getPatients = void 0;
 ...
 */
const verifyPatientAccess = async (userId, role, patientId) => {
    // Check if patient exists in users
    const pCheck = await (0, db_1.query)(`SELECT id, role FROM users WHERE id = $1`, [patientId]);
    ...
    const linkCheck = await (0, db_1.query)(`SELECT id FROM caregiver_patient_links WHERE caregiver_id = $1 AND patient_id = $2`, [userId, patientId]);
...
const getPatients = async (req, res) => {
    try {
      ...
      `;
        ...
        const patientsRes = await (0, db_1.query)(sql, params);
        return res.json({
            success: true,
...
const getPatientDetail = async (req, res) => {
    try {
        ...
        const userRes = await (0, db_1.query)(`SELECT u.id, u.full_name, u.email, u.phone_number, u.preferred_language,
       ...
       WHERE u.id = $1`, [patientId]);
        // Relationship
        const linkRes = await (0, db_1.query)(`SELECT relationship, can_edit_reminders FROM caregiver_patient_links WHERE patient_id = $1 AND caregiver_id = $2`, [patientId, userId]);
        // Cognitive profile (5 domains)
        const profileRes = await (0, db_1.query)(`SELECT patient_id, working_memory_score, processing_speed_score, attention_score,
       ...
       FROM cognitive_profiles WHERE patient_id = $1`, [patientId]);
        // Session aggregates
        const statsRes = await (0, db_1.query)(`SELECT COUNT(*)::int as total_sessions,
       ...
       WHERE patient_id = $1`, [patientId]);
        ...
        const sessionsRes = await (0, db_1.query)(`SELECT id, game_id, difficulty_level, score, accuracy_percentage, 
       ...
       LIMIT 15`, [patientId]);
        // Today's reminders
        const remindersRes = await (0, db_1.query)(`SELECT r.id, r.title, r.type, r.scheduled_time,
       ...
       ORDER BY r.scheduled_time ASC`, [patientId]);
        // Difficulty states
        const difficultyStatesRes = await (0, db_1.query)(`SELECT game_id, current_difficulty, ai_adjustment_notes, updated_at 
       FROM player_game_difficulty_states WHERE patient_id = $1`, [patientId]);
        // AI recommendation
        let recommendation = null;
        const cogProfile = profileRes.rows[0];
        if (cogProfile) {
            try {
                const aiRec = await aiServiceClient_1.AiServiceClient.getRecommendations({
...
exports.getPatientTrends = getPatientTrends;
const getPatientGames = async (req, res) => {
    try {
        const patientId = req.params.patientId || req.params.id;
        const userId = req.user?.id;
        const role = req.user?.role;
        ...
        // All active games
        const gamesRes = await (0, db_1.query)(`SELECT id, title as name, id as slug, primary_domain as domain, description, min_difficulty, max_difficulty 
       FROM games 
       ORDER BY id ASC`);
        // Current difficulty states
        const statesRes = await (0, db_1.query)(`SELECT game_id, current_difficulty, ai_adjustment_notes, updated_at 
       FROM player_game_difficulty_states 
       WHERE patient_id = $1`, [patientId]);
        const stateMap = new Map();
        statesRes.rows.forEach(st => stateMap.set(st.game_id, st));
        // Per-game analytics
        const gameAnalytics = await Promise.all(gamesRes.rows.map(async (g) => {
            const statsRes = await (0, db_1.query)(`SELECT COUNT(*)::int as sessions_completed,
                  COALESCE(ROUND(AVG(accuracy_percentage)::numeric, 1), 0) as avg_accuracy,
                  COALESCE(ROUND(AVG(average_reaction_time_ms)::numeric, 0), 0) as avg_reaction_time,
                  COALESCE(MAX(score), 0) as best_score,
                  MAX(client_created_at) as last_played_at
           FROM game_sessions
           WHERE patient_id = $1 AND game_id = $2`, [patientId, g.slug]);
            const recentSessionsRes = await (0, db_1.query)(`SELECT id, difficulty_level, score, accuracy_percentage, 
                  average_reaction_time_ms, mistakes_count, client_created_at
           FROM game_sessions
           WHERE patient_id = $1 AND game_id = $2
           ORDER BY client_created_at DESC
           LIMIT 5`, [patientId, g.slug]);
            // Difficulty history (progression over sessions)
            const diffHistoryRes = await (0, db_1.query)(`SELECT difficulty_level, client_created_at
           FROM game_sessions
           WHERE patient_id = $1 AND game_id = $2
           ORDER BY client_created_at ASC`, [patientId, g.slug]);
...
exports.getPatientGames = getPatientGames;
const getPatientReminders = async (req, res) => {
    try {
        ...
        const remindersRes = await (0, db_1.query)(`SELECT id, title, type, scheduled_time, is_active, days_of_week
       FROM reminders
       WHERE patient_id = $1 AND is_active = TRUE
       ORDER BY scheduled_time ASC`, [patientId]);
        // Today's status for each reminder
        const todayLogsRes = await (0, db_1.query)(`SELECT r.id as reminder_id, r.title, r.type, r.scheduled_time,
       ...
       ORDER BY r.scheduled_time ASC`, [patientId]);
        ...
        const historyRes = await (0, db_1.query)(`SELECT l.id, r.title, r.type, l.scheduled_date, r.scheduled_time,
...
const overrideDifficulty = async (req, res) => {
    try {
        const patientId = req.params.patientId || req.params.id;
        ...
        const result = await (0, db_1.query)(`INSERT INTO player_game_difficulty_states (patient_id, game_id, current_difficulty, ai_adjustment_notes, updated_at)
       VALUES ($1, $2, $3, $4, NOW())
       ON CONFLICT (patient_id, game_id)
       DO UPDATE SET current_difficulty = $3, ai_adjustment_notes = $4, updated_at = NOW()
       RETURNING *`, [patientId, gameId, newDifficulty, notes || `Manual calibration by caregiver (${req.user?.fullName})`]);
        return res.json({
            success: true,
...
exports.overrideDifficulty = overrideDifficulty;
const getAlerts = async (req, res) => {
    try {
      ...
      `;
        ...
        const changeAlertsRes = await (0, db_1.query)(changeSql, changeParams);
      ...
      `;
        ...
        const overdueMedsRes = await (0, db_1.query)(overdueSql, overdueParams);
        return res.json({
            success: true,
