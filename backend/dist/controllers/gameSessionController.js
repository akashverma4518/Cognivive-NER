...
exports.recordSession = exports.getGameState = exports.getGames = void 0;
const db_1 = require("../config/db");
const aiServiceClient_1 = require("../services/aiServiceClient");
const getGames = async (req, res) => {
    try {
        const result = await (0, db_1.query)('SELECT * FROM games ORDER BY title ASC');
...
const getGameState = async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`SELECT * FROM player_game_difficulty_states 
       WHERE patient_id = $1 AND game_id = $2`, [patientId, gameId]);
...
const recordSession = async (req, res) => {
    try {
        ...
        const { gameId, difficultyLevel, durationSeconds, score, accuracyPercentage, averageReactionTimeMs, mistakesCount = 0, consecutiveCorrect = 0, trials = [], telemetryPayload = {} } = req.body;
        ...
        const sessionRes = await (0, db_1.query)(`INSERT INTO game_sessions 
       ...
       RETURNING id, patient_id, game_id, difficulty_level, duration_seconds, score, accuracy_percentage, average_reaction_time_ms, client_created_at`, [
        ...
        const aiEvaluation = await aiServiceClient_1.AiServiceClient.evaluateSession({
            patient_id: patientId,
            game_id: gameId,
        ...
        await (0, db_1.query)(`INSERT INTO player_game_difficulty_states (patient_id, game_id, current_difficulty, ai_adjustment_notes, updated_at)
       VALUES ($1, $2, $3, $4, NOW())
       ON CONFLICT (patient_id, game_id)
       DO UPDATE SET current_difficulty = $3, ai_adjustment_notes = $4, updated_at = NOW()`, [patientId, gameId, nextDiff, adjustmentNotes]);
       ...
       WHERE patient_id = $9`, [
        ...
        const recentSessionsRes = await (0, db_1.query)(`SELECT game_id, accuracy_percentage, average_reaction_time_ms, mistakes_count, score, duration_seconds
       FROM game_sessions
       WHERE patient_id = $1
       ORDER BY client_created_at DESC LIMIT 5`, [patientId]);
        const patientProfileRes = await (0, db_1.query)(`SELECT baseline_activity_index FROM patient_profiles WHERE user_id = $1`, [patientId]);
        const baselineIndex = parseFloat(patientProfileRes.rows[0]?.baseline_activity_index || '50.0');
        const changeAnalysis = await aiServiceClient_1.AiServiceClient.detectChange(patientId, baselineIndex, recentSessionsRes.rows);
        // Update patient status in DB
        await (0, db_1.query)(`UPDATE patient_profiles SET status = $1 WHERE user_id = $2`, [changeAnalysis.status, patientId]);
        if (changeAnalysis.performance_change_flag) {
            await (0, db_1.query)(`UPDATE cognitive_profiles 
         SET performance_change_flag = TRUE, performance_change_notes = $1 
         WHERE patient_id = $2`, [changeAnalysis.notes, patientId]);
        }
