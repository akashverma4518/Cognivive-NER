...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    const userRes = await query(
      `INSERT INTO users (full_name, email, password_hash, role, phone_number, preferred_language, ner_region, age, gender)
    ...
    if (targetRole === 'ELDER') {
      await query(
        `INSERT INTO patient_profiles (user_id, emergency_contact_name, emergency_contact_phone, ner_region, age, gender)
         VALUES ($1, $2, $3, $4, $5, $6)
      ...
      for (const gid of allSevenGames) {
        await query(
          `INSERT INTO player_game_difficulty_states (patient_id, game_id, current_difficulty)
           VALUES ($1, $2, 1) ON CONFLICT (patient_id, game_id) DO NOTHING`,
          [user.id, gid]
        );
      }
    }
