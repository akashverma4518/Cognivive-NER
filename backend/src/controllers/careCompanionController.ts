...
async function resolveAuthorizedPatientId(req: Request, requestedPatientId?: string): Promise<string | null> {
  const user = req.user;
  if (!user) return null;
  ...
  if (user.role === 'CAREGIVER') {
    ...
    const linkRes = await query(
      ...
      [user.id, requestedPatientId]
    );
    if (linkRes.rows.length === 0) return null;
...
export const getMyDayOverview = async (req: Request, res: Response) => {
  try {
    const patientId = await resolveAuthorizedPatientId(req, req.query.patientId as string);
    if (!patientId) {
...
export const getWellnessActivities = async (req: Request, res: Response) => {
  try {
    const patientId = await resolveAuthorizedPatientId(req, req.query.patientId as string);
    if (!patientId) {
...
export const logWellnessActivity = async (req: Request, res: Response) => {
  try {
    const user = req.user;
    ...
    if (user.role === 'CAREGIVER') {
      ...
      if (!valid) return res.status(403).json({ success: false, message: 'Unauthorized patient link' });
    }
...
export const getFamilyMemories = async (req: Request, res: Response) => {
  try {
    const patientId = await resolveAuthorizedPatientId(req, req.query.patientId as string);
    if (!patientId) {
      return res.status(403).json({ success: false, message: 'Unauthorized patient access.' });
...
export const addFamilyMemory = async (req: Request, res: Response) => {
  try {
    const user = req.user;
    if (!user) return res.status(401).json({ success: false, message: 'Authentication required' });
...
export const deleteFamilyMemory = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const user = req.user;
    if (!user) return res.status(401).json({ success: false, message: 'Authentication required' });
    ...
    const itemRes = await query('SELECT patient_id FROM family_memory_items WHERE id = $1', [id]);
    ...
    const valid = await resolveAuthorizedPatientId(req, targetPatientId);
    if (!valid) return res.status(403).json({ success: false, message: 'Unauthorized to delete memory.' });

    await query('DELETE FROM family_memory_items WHERE id = $1', [id]);
...
// 4. Caregiver Notes
export const getCaregiverNotes = async (req: Request, res: Response) => {
  try {
    const user = req.user;
    ...
    if (!valid) return res.status(403).json({ success: false, message: 'Unauthorized patient access.' });
...
export const addCaregiverNote = async (req: Request, res: Response) => {
  try {
    const user = req.user;
    if (!user || user.role !== 'CAREGIVER') {
...
export const getActionCenterAlerts = async (req: Request, res: Response) => {
  try {
    const user = req.user;
...
export const getPatientProgressReport = async (req: Request, res: Response) => {
  try {
    ...
    const cpRes = await query('SELECT * FROM cognitive_profiles WHERE patient_id = $1', [patientId]);
